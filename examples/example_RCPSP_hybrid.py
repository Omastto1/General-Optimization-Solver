...
def fitness_func_forward(instance, x, out):
    ...
    resource_usage_over_time = np.zeros(
        (instance.no_renewable_resources, sum(instance.durations)))
    ...
    while len(unscheduled_jobs) > 0:
        ...
        schedulable_jobs = [j for j in unscheduled_jobs if all(
            (pred - 1) not in unscheduled_jobs for pred in instance.predecessors[j])]
        ...
        for job in schedulable_jobs:
            ...
            start_time = max(start_times[pred-1] + instance.durations[pred-1]
    ...
    resource_violations = np.max(
        resource_usage_over_time - np.array(instance.renewable_capacities)[:, np.newaxis], axis=1)
...
naive_GA_solver_forward = RCPSPGASolver(
    algorithm, fitness_func_forward, ("n_gen", 5), seed=1, solver_name="naive GA forward")
...
brkga_fitness_value, brkga_assignment, brkga_solution = naive_GA_solver_forward.solve(
    instance, visualize=False, validate=True)

cp_bins_used, cp_solution_variables, cp_solution = cp_solver._solve(
    instance, validate=True, visualize=True, force_execution=True, initial_solution=brkga_assignment)
