    ...
    def _solve(self, instance, validate=False, visualize=False, force_execution=False, update_history=True):
        class MMRCPSP(ElementwiseProblem):
            ...
            def __init__(self, instance, fitness_func):
                ...
                super().__init__(n_var=instance.no_jobs * 4, n_obj=1, n_constr=instance.no_renewable_resources + instance.no_non_renewable_resources, xu=100, xl=0)
                print("WARNING: MMRCPSP GA SOLVER SUPPORTS ONLY 3 DIFFERENT MODES FOR EACH JOB")
        ...
        if res.F is not None:
            ...
            export = {"task_mode_assignment": [{"start": start_times[job], "end": start_times[job] + instance.durations[job][selected_mode], "mode": selected_mode, "name": f"Task_{job}_{selected_mode}"} for job, selected_mode in sorted(selected_modes.items())]}
