...
def load_patterson(instance_path, verbose=False):
    ...
    with open(instance_path) as file:
        ...
        number_of_jobs, number_of_renewable_resources = [
            int(number.strip()) for number in line.split(" ") if len(number.strip()) > 0]
        parsed_input["number_of_jobs"] = number_of_jobs
        parsed_input["resources"] = {}

        # 3. line: Availability for each renewable resource
        resource_availabilities = [int(number.strip()) for number in file.readline(
        ).split(" ") if len(number.strip()) > 0]
        ...
        line = [int(number.strip())
                for number in line.split(" ") if len(number.strip()) > 0]
        # duration, renewable resources consumption, number of succesors
        ending_line = [0] + [0] * number_of_renewable_resources + [0]
        while line != ending_line:
            ...
            duration = line[0]
            renewable_resource_consumption = line[1:1 +
            ...
            successors = line[2+int(number_of_renewable_resources):2 +
                              int(number_of_renewable_resources)+int(number_of_successors)]
            ...
            assert len(
                successors) == number_of_successors, "Number of successors does not match the number of successors given in the line"
    ...
    terminal_node_resources_consumption = {
        f"R{resource_index}": 0 for resource_index in range(1, number_of_renewable_resources+1)}
    job_specification = {"job_nr": number_of_jobs, "no_modes": 1, "modes": [
        {"mode": 1, "duration": 0, "request_duration": terminal_node_resources_consumption}], "number_of_successors": 0, "successors": []}
    parsed_input["job_specifications"].append(job_specification)
    parsed_input
...
def load_patterson_solution(file_path, instance_name):
    ...
    instance_result = instances_results[instances_results["Instance name"]
                                        == instance_name]
